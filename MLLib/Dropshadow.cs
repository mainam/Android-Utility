...
namespace MLLib
{
	...
	public class Dropshadow : Form
	{
		...
		public Dropshadow(Form f)
		{
            ...
            Owner.ResizeBegin += (sender, args) => this.Hide();
            Owner.ResizeEnd += (sender, args) => this.Show();
		...
		public void RefreshShadow(bool redraw = true)
		{
			...
			// 设置显示区域
			//Region r = Region.FromHrgn(Win32API.CreateRoundRectRgn(0, 0, Width, Height, BorderRadius, BorderRadius));
		...
		public void SetBitmap(Bitmap bitmap, byte opacity = 255)
		{
			...
			try
			{
				...
				var size = new Win32API.Size(bitmap.Width, bitmap.Height);
				var pointSource = new Win32API.Point(0, 0);
				var topPos = new Win32API.Point(Left, Top);
				...
				Win32API.UpdateLayeredWindow(Handle, screenDc, ref topPos, ref size, memDc, ref pointSource, 0, ref blend,
					Win32API.ULW_ALPHA);
			} finally
			{
				...
				if (hBitmap != IntPtr.Zero)
				{
					...
					//Windows.DeleteObject(hBitmap); // The documentation says that we have to use the Windows.DeleteObject... but since there is no such method I use the normal DeleteObject from Win32API GDI and it's working fine without any resource leak.
	...
	// class that exposes needed Win32API gdi functions.
