...
*/
        ...
        /// <summary>
        /// Force-Stop an package from the device.
        /// </summary>
        /// <param name="packageName">Name of the package.</param>
        public void PullFile(String remotefile, String localfile, AdbCommand adb)
        {
            adb.ExecuteCommandSync("adb -s " + this.SerialNumber + " pull " + remotefile + " " + localfile, true);
        }
        ...
        /// </summary>
        /// <param name="packageName">Name of the package.</param>
        public void ExecuteAdbCommand(AdbCommand adb, string command, bool wait)
        {
            adb.ExecuteCommandSync("adb -s " + this.SerialNumber + " " + command, wait);
        }
        ...
        {
            InstallReceiver receiver = new InstallReceiver();
            ExecuteShellCommand(String.Format(" am force-stop {0}", packageName), receiver);
            if (!String.IsNullOrEmpty(receiver.ErrorMessage))
            {
                throw new Exception(receiver.ErrorMessage);
            }
        }


        /// <summary>
        /// Clear user data an package from the device.
        /// </summary>
        /// <param name="packageName">Name of the package.</param>
        public void ClearDataPackage(String packageName)
        {
            InstallReceiver receiver = new InstallReceiver();
            //ExecuteShellCommand(String.Format(" pm clear {0}", packageName), receiver);
            ExecuteShellCommand(String.Format(" pm clear {0}", packageName), receiver);
            if (!String.IsNullOrEmpty(receiver.ErrorMessage))
            {
                throw new Exception(receiver.ErrorMessage);
